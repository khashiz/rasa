...
<?php else : ?>
	<ul aria-labelledby="language_picker_des_<?php echo $module->id; ?>" class="uk-flex uk-height-1-1 langSwitcher mod-languages__list <?php echo $params->get('inline', 1) ? 'lang-inline' : 'lang-block'; ?>">
	...
	<?php foreach ($list as $language) : ?>
		...
		<?php if (!$language->active) : ?>
			<li class="uk-flex uk-height-1-1">
				<a <?php echo $lbl; ?> href="<?php echo htmlspecialchars_decode(htmlspecialchars($language->link, ENT_QUOTES, 'UTF-8'), ENT_NOQUOTES); ?>" class="uk-text-tiny uk-flex uk-flex-middle uk-height-1-1">
					...
					<?php if ($params->get('image', 1)) : ?>
						...
						<?php endif; ?>
					<?php else : ?>
                        <span class="font"><?php echo $params->get('full_name', 1) ? $language->title_native : strtoupper($language->sef); ?></span>
					<?php endif; ?>
                </a>
			</li>
		<?php elseif ($params->get('show_active', 1)) : ?>
